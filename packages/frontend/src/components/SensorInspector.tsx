...
import type { SensorAxisId, SensorState } from '../stores/mechanism.js';
...
export function SensorInspector({ sensorId }: { sensorId: string }) {
  ...
  const datum = useMechanismStore((s) => (sensor ? s.datums.get(sensor.datumId) : undefined));
  ...
  const channelDefinitions = useMemo(() => {
    ...
    switch (sensor.type) {
      ...
      case 'gyroscope':
        return [
          {
            channelId: prefix + 'angular_velocity',
      ...
      case 'tachometer':
        return [{ channelId: prefix + 'rpm', label: 'RPM', unit: 'rpm', type: 'scalar' as const }];
      case 'encoder':
        return [
          {
            channelId: prefix + 'position',
            label: 'Position',
            unit: joint?.type === 'prismatic' ? 'm' : 'rad',
            type: 'scalar' as const,
          },
          {
            channelId: prefix + 'velocity',
            label: 'Velocity',
            unit: joint?.type === 'prismatic' ? 'm/s' : 'rad/s',
            type: 'scalar' as const,
          },
        ];
      default:
