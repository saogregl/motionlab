import { Color3, DirectionalLight, HemisphericLight, type Scene, Vector3 } from '@babylonjs/core';
...
export function createLightingRig(scene: Scene): LightingRig {
  ...
  const keyLight = new DirectionalLight('key_light', new Vector3(-1, -2, 1).normalize(), scene);
  keyLight.intensity = 0.9;
  keyLight.diffuse = Color3.White();
  keyLight.specular = Color3.White();

  // Fill light — pure white, softer
  const fillLight = new DirectionalLight('fill_light', new Vector3(1, -1, -0.5).normalize(), scene);
  fillLight.intensity = 0.45;
  fillLight.diffuse = Color3.White();
  fillLight.specular = Color3.White();

  // Rim light — accent from behind
  const rimLight = new DirectionalLight('rim_light', new Vector3(0.2, -0.5, -1).normalize(), scene);
  ...
  const ambientLight = new HemisphericLight('ambient_light', new Vector3(0, 1, 0), scene);
