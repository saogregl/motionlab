...
- `PrepareFacePickingCommand`: optional authoring-performance hint that asks the engine to load geometry topology and precompute cached face metadata for the listed geometry IDs before the first face-aware click
- `PrepareFacePickingResult`: reports which geometry IDs were warmed successfully; hover picking still remains viewport-local
...
Protocol constants: `PROTOCOL_NAME = "motionlab"`, `PROTOCOL_VERSION = 6` (defined in `packages/protocol/src/version.ts`).
